...
from crc.utils.os_utils import magenta, cyan, red
...
def print_with_rank(*args, **kwargs):
    rank = get_rank()
    disable_ranks = kwargs.get('disable_ranks', [])
    if rank not in disable_ranks:
        colors = ['red', 'cyan', 'magenta', 'blue', 'green', 'yellow']
        color = colors[rank % len(colors)]
        args = [colored(s, color) for s in args]
...
#             ret[k]=cat_outputs([o[k] for o in outputs])
#         return ret
