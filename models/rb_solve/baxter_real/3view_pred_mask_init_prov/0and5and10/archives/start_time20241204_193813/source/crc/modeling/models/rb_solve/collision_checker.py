...
class CollisionChecker:
    ...
    def move_to_pose(self, pose):
        ...
        result = self.planner.plan_qpos(pose, self.robot.get_qpos(), time_step=1 / 250,
                                        use_point_cloud=True)
        if result['status'] != "Success":
            # print(result['status'])
            return -1, None
    ...
    def move_to_qpos(self, target_qpos, mask=[],
        ...
        evaltime("collision_checker_start")
        target_qpos = np.array(target_qpos)
        current_qpos = self.robot.get_qpos()
        self.planner.plan_qpos([target_qpos], current_qpos, )
        # self.planner.planning_world.set_use_point_cloud(use_point_cloud)
        # self.planner.planning_world.set_use
        ...
        evaltime("collision_checker_step1")
        self.planner.robot.set_qpos(current_qpos, True)
        self.planner.plan_qpos()
        ...
        self.planner.robot.set_qpos(current_qpos, True)
        evaltime("collision_checker_step2")
