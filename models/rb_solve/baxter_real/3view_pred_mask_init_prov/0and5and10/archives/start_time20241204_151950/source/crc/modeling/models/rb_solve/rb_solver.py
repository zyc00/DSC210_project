...
class RBSolver(nn.Module):
    def __init__(self, cfg):
        ...
        for link_idx, mesh_path in enumerate(mesh_paths):
            ...
            if self.cfg.optim_mesh_scale is True:
                setattr(self, f'mesh_scale_{link_idx}', nn.Parameter(torch.tensor([1.0], dtype=torch.float32), requires_grad=True))
        ...
        if self.cfg.optim_eep:
            if self.cfg.optim_eep_mask != []:
                ...
                if self.cfg.optim_eep_xy:
                    self.eep = nn.Parameter(torch.tensor([0, 0, 0.005, 0, 0, 0], dtype=torch.float32), requires_grad=True)
                else:
    ...
    def forward(self, dps):
        vis3d = Vis3D(xyz_pattern=("x", "-y", "-z"), out_folder="dbg", sequence_name="rbsolver_forward",
                        ...
                        if self.cfg.optim_eep_mask != []:
                            ...
                            if self.cfg.optim_eep_xy:
                                ...
                                if self.cfg.optim_eep_rot:
                                    ...
                                    eep = torch.cat([torch.zeros(2, device=self.eep.device, dtype=torch.float),
                                                     self.eep,
                                                     torch.zeros(3, device=self.eep.device, dtype=torch.float), ], dim=0)
                                    Te_e2ep = se3_exp_map(eep[None]).permute(0, 2, 1)[0]
        ...
        if self.cfg.optim_eep:
            if self.cfg.reg_loss_weight > 0:
                if self.cfg.optim_eep_xy:
                    reg_loss = self.cfg.reg_loss_weight * (self.eep - torch.tensor([0, 0, 0.005, 0, 0, 0], device='cuda')).norm()
                else:
                    reg_loss = self.cfg.reg_loss_weight * (self.eep - torch.tensor([0.005, 0, 0, 0], device='cuda')).norm()
