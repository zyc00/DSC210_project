...
from crc.utils.sam_api import SAMAPI
...
class RBSolverIterRealmanTrainer(BaseTrainer):
    def __init__(self, cfg):
        ...
        self.qpos_pool = self.cfg.model.rbsolver_iter.qpos_pool
        if self.cfg.model.rbsolver_iter.use_realman.enable is True:
            # todo setup realman api
            self.arm = self.cfg.model.rbsolver_iter.use_realman.arm  # left or right
            self.camera = 'right' if self.arm == 'left' else 'left'
            pass
    ...
    def train(self, epoch):
        ...
        metric_ams = {}
        if epoch == 0:
            assert len(self.train_dl) == 1
            bar = tqdm.tqdm(self.train_dl, leave=False) if is_main_process() and len(
                self.train_dl) > 1 else self.train_dl
    ...
    def capture_data(self):
        ...
        if self.cfg.model.rbsolver_iter.use_realman.enable is True:
            loguru.logger.info("Use realman")
            self.realman_capture_data()
            print()
    ...
    def sapien_sim_data(self):
        ...
        if cfg.add_desk_cube.enable:
            ...
            mtl.set_base_color(cfg.add_desk_cube.color + [1])
        ...
        if cfg.Tc_c2b_npy_path != "":
            Tc_c2b = np.load(cfg.Tc_c2b_npy_path)[cfg.Tc_c2b_index]
        else:
            Tc_c2b = np.array(cfg.Tc_c2b)
        # Tc_c2b = np.array(cfg.Tc_c2b)
        camera_pose = np.linalg.inv(Tc_c2b)
        np.savetxt(osp.join(outdir, "Tc_c2b.txt"), Tc_c2b)
        ...
        camera_mount_actor.set_pose(sapien.Pose(camera_pose @ coord_convert))
        ...
        # for qpose in tqdm(qposes, leave=False):
        qpose = self.qposes[-1]
        ...
        desk_id = np.unique(seg_labels[:, :, 1]).max()
        rendered_mask = np.logical_and(seg_labels[..., 1] > 0, seg_labels[..., 1] < desk_id)
        ...
        if self.cfg.model.rbsolver_iter.qpos_noise > 0:
            # qpos_noise = np.random.normal(0, self.cfg.model.rbsolver_iter.qpos_noise / 57, size=qpose.shape)
            qpos_noise = (np.random.random(qpose.shape) * 2 - 1) * self.cfg.model.rbsolver_iter.qpos_noise
            qpose = qpose + qpos_noise / 57

        np.savetxt(osp.join(outdir, f"qpos/{index:06d}.txt"), qpose)

    def segment(self, index):
        outdir = self.cfg.model.rbsolver_iter.data_dir
        image_path = osp.join(outdir, f"color/{self.camera}/{index:06d}.png")
        ...
        assert segmentor == "sam"
        from crc.utils.prompt_drawer import PromptDrawer
        prompt_drawer = PromptDrawer(sam_checkpoint="third_party/segment_anything/sam_vit_h_4b8939.pth",
                                     screen_scale=3.0)
        ...
        #         init_Tc_c2b = np.array(self.cfg.model.rbsolver.init_Tc_c2b)
        #         prev_Tc_c2b = init_Tc_c2b
        #     else:
        #         prev_Tc_c2b = utils_3d.se3_exp_map(self.model.dof[None]).permute(0, 2, 1)[0].detach().cpu().numpy()
        #     qpos = np.loadtxt(osp.join(outdir, f"qpos/{index:06d}.txt"))
        #     dbg = True
        #     rendered_masks = render_api.nvdiffrast_render_xarm_api("data/xarm7_textured_with_gripper_reduced_dof.urdf",
        #                                                            prev_Tc_c2b, qpos.tolist() + [0, 0], H, W, K, return_sum=False)
        #     sam_masks = []
        #     autosam_links = self.cfg.model.rbsolver_iter.autosam_links
        ...
        #                 sam_mask, dbg_img = sam_api.SAMAPI.segment_api(img, joint_mask, box_scale=self.cfg.model.rbsolver_iter.autosam_box_scale,
        #                                                                point_center=self.cfg.model.rbsolver_iter.autosam_point_center,
        #                                                                random_points=self.cfg.model.rbsolver_iter.autosam_random_points,
        #                                                                dbg=True)
        ...
        outpath = osp.join(outdir, "pred_mask", self.camera, osp.basename(image_path))
    ...
    def realman_capture_data(self):
        # todo send capture data cmd to rm server, then sync data to local
        # import pyrealsense2 as rs
        outdir = self.cfg.model.rbsolver_iter.data_dir
        os.makedirs(outdir, exist_ok=True)
        os.makedirs(osp.join(outdir, "color/head"), exist_ok=True)
        os.makedirs(osp.join(outdir, "color/left"), exist_ok=True)
        os.makedirs(osp.join(outdir, "color/right"), exist_ok=True)
        os.makedirs(osp.join(outdir, "depth"), exist_ok=True)
        os.makedirs(osp.join(outdir, "gt_mask"), exist_ok=True)
        os.makedirs(osp.join(outdir, "qpos"), exist_ok=True)
        ...
        vis3d.set_scene_id(index)
        qpose = self.qposes[-1]
        vis3d.add_realman(np.array(qpose.tolist() + [0] * 6))
        # arm = self.arm
        speed = self.cfg.model.rbsolver_iter.use_realman.speed
        if len(self.qposes) == 1:
            if self.arm == 'left':
                api_control_qpose = qpose[3::2]
            else:
                api_control_qpose = qpose[4::2]
            api_control_qpose = ','.join(list(map(str, np.rad2deg(api_control_qpose))))
            cmd = f"curl -X POST http://192.168.31.175:2525/movej -F arm={self.arm} -F qpos='{api_control_qpose}' -F speed={speed}"
            print(cmd)
            os.system(cmd)
            ...
            if self.camera == 'left':
                ...
                api_control_qpose = qpose[4::2]
            api_control_qpose = ','.join(list(map(str, np.rad2deg(api_control_qpose))))
            cmd = f"curl -X POST http://192.168.31.175:2525/movej -F arm={self.camera} -F qpos='{api_control_qpose}' -F speed={speed}"
            print(cmd)
            os.system(cmd)
            ...
            if not self.cfg.model.rbsolver_iter.use_realarm.speed_control:
                ...
                print()
            print()
        time.sleep(self.cfg.model.rbsolver_iter.use_realman.wait_time)

        cmd = "curl -X POST http://192.168.31.175:2525/capture -o tmp.zip"
        os.system(cmd)
        os.system(f"unzip tmp.zip -d {outdir}")
        os.system(f"mv {outdir}/color_image_head.png {outdir}/color/head/{index:06d}.png")
        os.system(f"mv {outdir}/color_image_left.png {outdir}/color/left/{index:06d}.png")
        os.system(f"mv {outdir}/color_image_right.png {outdir}/color/right/{index:06d}.png")
        os.system(f"mv {outdir}/qpos.txt {outdir}/qpos/{index:06d}.txt")
        print()

    def explore_next_state(self, explore_it, to_zero=False):
        if len(self.qpos_pool) > 0:
            next_qpos = self.qpos_pool[0]
            self.qposes = np.concatenate([self.qposes, np.array(next_qpos)[None]], axis=0)
            self.qpos_pool = self.qpos_pool[1:]
        else:
            ...
            dps['to_zero'] = to_zero
            for i in range(self.cfg.model.rbsolver_iter.explore_iters):
                outputs, _ = space_explorer(dps)
                loguru.logger.info(f"explore iter {i}")
                loguru.logger.info(f"var {outputs['variance'].item()}")
                dps['center_qpos'] = outputs['qpos']
    ...
    def initialize_Tc_c2b(self):
        ...
        if init_method == 'manual':
            assert self.cfg.model.rbsolver.init_Tc_c2b != [], "init_Tc_c2b is empty"
            ...
            if self.cfg.use_xarm is True:
                ...
                output = self.cfg.model.rbsolver.init_Tc_c2b
            ...
            if len(glob.glob(osp.join(db_dir, "*png"))) == 0:
                init_Tc_c2b = np.array(self.cfg.model.rbsolver_iter.meshloc_init_Tc_c2b)
            else:
                ref_img_paths = sorted(glob.glob(osp.join(db_dir, "*png")))
                ref_Tc_c2b_paths = sorted(glob.glob(osp.join(db_dir, "*txt")))
                assert len(ref_img_paths) == len(ref_Tc_c2b_paths)
                if self.cfg.model.rbsolver_iter.meshloc_use_all_imgs is False:
                    ref_img_paths = [ref_img_paths[0]]
                    ref_Tc_c2b_paths = [ref_Tc_c2b_paths[0]]
                for ref_img_path, ref_Tc_c2b_path in zip(ref_img_paths, ref_Tc_c2b_paths):
                    # ref_img_path = ref_img_paths[0]
                    ref_img = imageio.imread_v2(ref_img_path)
                    H, W = ref_img.shape[:2]
                    # ref_Tc_c2b_path = ref_Tc_c2b_paths[0]
                    ref_Tc_c2b = np.loadtxt(ref_Tc_c2b_path)
                    # mesh = trimesh.load("data/xarm7_init_qpos.ply")
                    mesh = trimesh.load("data/xarm7_zeropos.ply")
                    outdir = self.cfg.model.rbsolver_iter.data_dir
                    K = np.loadtxt(osp.join(outdir, "K.txt"))
                    ref_mask = render_api.nvdiffrast_render_mesh_api(mesh, ref_Tc_c2b, H, W, K, anti_aliasing=False)
                    ...
                    kpts0, kpts1, conf = dkm_api.get_feature_matching(ref_img_path, tgt_img_path, mask0=ref_mask,
                                                                      dbg=False, resize_factor=0.25, num=2000)
                    dkm_api.DKMHelper.reset()
                    wis3d = Wis3D(
                        out_folder="dbg",
                        sequence_name="initialize_Tc_c2b",
                        xyz_pattern=("x", "-y", "-z"),
                        enable=True
                    )
                    wis3d.add_keypoint_correspondences(Image.open(ref_img_path), Image.open(tgt_img_path), kpts0, kpts1,
                                                       metrics={'mconf': conf})
                    depth = render_api.pyrender_render_mesh_api(mesh, ref_Tc_c2b, 720, 1280, K, return_depth=True)
                    fu, fv, cu, cv = K[0, 0], K[1, 1], K[0, 2], K[1, 2]
                    pc = utils_3d.depth_to_rect(fu, fv, cu, cv, depth)
                    matched_pts0 = pc.reshape(H, W, 3)[kpts0.astype(int)[:, 1], kpts0.astype(int)[:, 0]]
                    matched_pts0_ca = utils_3d.transform_points(matched_pts0, np.linalg.inv(ref_Tc_c2b))
                    ...
                    init_Tc_c2b = tgt_Tc_c2b
                np.savetxt(osp.join(outdir, "init_Tc_c2b.txt"), init_Tc_c2b)
                evaltime("")
        else:
            raise NotImplementedError()
        self.cfg.defrost()
