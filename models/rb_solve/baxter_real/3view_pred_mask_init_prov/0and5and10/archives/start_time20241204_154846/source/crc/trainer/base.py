...
from crc.data.samplers.ordered_distributed_sampler import OrderedDistributedSampler
# from crc.loss.build import build_loss_function
from crc.modeling.build import build_model
from crc.modeling.layers.ddp import DistributedDataParallel as EDDP
from crc.modeling.layers.norm.syncbatchnorm import SyncBatchNorm as ESBN
from crc.solver.build import make_optimizer, make_lr_scheduler
from crc.trainer.utils import *
from crc.utils.os_utils import red
from crc.utils.tb_utils import get_summary_writer
...
class BaseTrainer:
    ...
    def train(self, epoch):
        ...
        if is_main_process():
            metric_msgs = ['epoch %d, train, loss %.6f, time %s' % (
    ...
    def val(self, epoch):
        ...
        epoch_time = format_time(time.time() - begin)
        if is_main_process():
            metric_msgs = ['epoch %d, val, loss %.6f, time %s' % (
                epoch, loss_meter.avg, epoch_time)]
            for metric, v in metric_ams.items():
                metric_msgs.append('%s %.4f' % (metric, v.avg))
            s = ', '.join(metric_msgs)
    ...
    def fit(self):
        ...
        for epoch in range(self.begin_epoch, num_epochs):
            ...
            if not self.save_every and epoch % self.cfg.solver.val_freq == 0:
                ...
                synchronize()
            elif self.save_every and epoch % self.save_freq == 0:
    ...
    def to_distributed(self):
        ...
        DDP = DistributedDataParallel if self.cfg.solver.ddp_version == 'torch' else EDDP
        ...
        dist_sampler_type = self.cfg.solver.dist.sampler.type
        dist_sampler_shuffle = self.cfg.solver.dist.sampler.shuffle
        if dist_sampler_type == 'pytorch':
            ...
            from crc.data.samplers.neucon_sampler import DistributedSampler as NeuConSampler
    ...
    def resume(self):
        # if self.cfg.solver.force_no_resume:
        #     return
    ...
    #     if get_rank() == 0:
    #         timestamp_file = osp.join(self.output_dir, 'timestamp.txt')
    #         if osp.exists(timestamp_file):
    #             with open(timestamp_file)as f:
    #                 timestamp = f.read()
    #         else:
    #             timestamp = 'archive_time_' + str(datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
    #         archive_dir = osp.join(self.output_dir, timestamp)
    #         for file in os.listdir(self.output_dir):
    #             if file != timestamp:
    #                 os.system(f'mv {osp.join(self.output_dir, file)} {archive_dir}')
    # models = glob.glob(osp.join(self.output_dir, "*.pth"))
    # try:
    #     if len(models) > 0 and get_rank() == 0:
    #         if click.confirm(f"Remove {len(models)} models?"):
    #             os.system(f'rm {self.output_dir}/*.pth')
    # except:
    #     pass
    # synchronize()
