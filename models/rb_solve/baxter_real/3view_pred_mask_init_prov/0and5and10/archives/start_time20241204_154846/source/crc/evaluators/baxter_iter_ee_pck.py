...
from crc.utils import utils_3d, comm


@EVALUATORS.register('baxter_iter_ee_pck')
def build(cfg):
    def f(x, trainer):
        if comm.get_rank() == 0:
            ckpt_paths = glob.glob(osp.join(trainer.output_dir, "model_*.pth"))
            explore_iters = trainer.cfg.solver.explore_iters
            num_epochs = trainer.cfg.solver.num_epochs
            for ei in range(explore_iters):
                ckpt_path = osp.join(trainer.output_dir, f"model_iteration_{((ei + 1) * num_epochs):06d}.pth")
                ...
                # load gt
                data_dir = osp.expanduser("./baxter_real/baxter-real-dataset")
                ...
                eval_dump_path = osp.join(trainer.output_dir, f"evaluation/err2d{ei}.txt")
                ...
                eval_dump_path = osp.join(trainer.output_dir, f"evaluation/err3d{ei}.txt")
                os.makedirs(osp.dirname(eval_dump_path), exist_ok=True)
