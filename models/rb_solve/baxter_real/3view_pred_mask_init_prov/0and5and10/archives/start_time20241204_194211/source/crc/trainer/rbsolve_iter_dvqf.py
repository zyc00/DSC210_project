...
class RBSolverIterTrainerDVQF(BaseTrainer):
    ...
    def sapien_sim_data(self):
        ...
        if cfg.add_chessboard:
            checkerboard.set_pose(sapien.Pose(Tb_b2m))
        for i in range(len(Tc_c2es)):
            ...
            for qpose in tqdm.tqdm(qposes, leave=False):
                ...
                # distort image
                distort_coeffs = np.array([0.002, 0.002, 0.002, 0.002])
