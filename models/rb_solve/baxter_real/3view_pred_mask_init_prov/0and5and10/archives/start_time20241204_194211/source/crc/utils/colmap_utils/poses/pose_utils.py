...
def gen_poses(basedir, match_type, factors=None, force_colmap=False, remote=False,cam_model='OPENCV'):
    files_needed = ['{}.bin'.format(f) for f in ['cameras', 'images', 'points3D']]
    if os.path.exists(os.path.join(basedir, 'sparse/0')):
        files_had = os.listdir(os.path.join(basedir, 'sparse/0'))
    else:
        files_had = []
    if not all([f in files_had for f in files_needed]) or force_colmap:
        print('Need to run COLMAP')
        run_colmap(basedir, match_type, remote,cam_model)
    else:
