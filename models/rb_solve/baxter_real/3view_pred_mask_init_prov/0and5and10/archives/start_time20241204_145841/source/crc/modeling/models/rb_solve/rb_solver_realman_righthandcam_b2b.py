...
class RBSolverRealmanRightHandCamB2B(nn.Module):
    ...
    def forward(self, dps):
        ...
        for bid in range(batch_size):
            all_link_si = []
            qpos = qposes[bid]
            Tbase_base2re = torch.from_numpy(vis3d.realman_sk.compute_forward_kinematics(
                qpos.cpu().numpy(), 17).to_transformation_matrix()).float().cuda()
            ...
            for link_idx in range(self.nlinks):
                ...
                Tre_re2l = Tbase_base2re.inverse() @ Tbase_base2rb @ B2B @ Tbase_base2rb.inverse() @ Tbase_base2l
                # Tle_le2l = Tbase_base2le.inverse() @ Tbase_base2rb @ Tbase_base2rb.inverse() @ Tbase_base2l
                Tc_c2l = Tc_c2b @ Tre_re2l  # Tc_c2b is actually Tc_c2re
