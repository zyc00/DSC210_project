...
from crc.utils.comm import get_rank
from crc.utils.os_utils import magenta
from crc.utils.pn_utils import random_choice, to_array, clone_if_present, numel
...
class Vis3D(Wis3D):
    ...
    def __init__(self, xyz_pattern=None, out_folder='dbg',
        ...
        if enable is True:
            ...
            super().__init__(out_folder, sequence_name, xyz_pattern)
            ...
            if auto_increase:
                ...
                scene_id = 0
            print(magenta(f'Set up Vis3D for {sequence_name}: {scene_id}'))
    ...
    def add_deformation_graph(self, graph, colors=None, name=None):
        ...
        if not self.enable:
            return
        from crc.structures.deformation_graph import DeformationGraph
    ...
    def add_flow2d(self, flow, name=None):
        ...
        :return:
        """
        from crc.utils.flow_viz import flow_to_image
    ...
    def add_xarm(self, qpos=None, Tw_w2B=None, add_local_coord=False, name=""):
        """

        Parameters
        ----------
        qpos: 13-dim ndarray, radian
        ...
        if qpos is None:
            qpos = np.zeros(13)
        qpos = to_array(qpos).tolist() + [0] * (13 - len(qpos))
    ...
    def xarm_sk(self):
        if Vis3D._xarm_sk is None:
            ...
            urdf_path = "data/sapien_packages/xarm7/xarm_urdf/xarm7_gripper_stl.urdf"
            sk = SAPIENKinematicsModelStandalone(urdf_path)
            Vis3D._xarm_sk = sk
        return Vis3D._xarm_sk

    def add_panda(self, qpos, Tw_w2B=None, add_local_coord=False, name=""):
        """
        Parameters
        ----------
        ...
        for link in range(num_links):
            ...
            pq = sk.compute_forward_kinematics(qpos, link)
            pose = Rt_to_pose(transforms3d.quaternions.quat2mat(pq.q), pq.p)
            from crc.structures.panda_mapping import link_name_mesh_path_mapping
            mesh_path = link_name_mesh_path_mapping[link_name]
            if mesh_path == "": continue
            mesh = trimesh.load_mesh(mesh_path)
            add_name = link_name if name == "" else name + "_" + link_name
    ...
    @property
    def panda_sk(self):
        if Vis3D._panda_sk is None:
            from crc.structures.sapien_kin import SAPIENKinematicsModelStandalone
            urdf_path = "./ManiSkill2/mani_skill2/assets/descriptions/panda_v2.urdf"
            sk = SAPIENKinematicsModelStandalone(urdf_path)
            Vis3D._panda_sk = sk
        return Vis3D._panda_sk

    def add_baxter(self, qpos, Tw_w2B=None, add_local_coord=False, name=""):
        """
                Parameters
                ----------
                qpos: 7-dim ndarray, radian, last 7 joints in SAPIEN
                Tw_w2B: base in world
                add_local_coord
                name
                Returns
        ...
        for link in range(num_links):
            ...
            mesh_path = link_name_mesh_path_mapping.get(link_name, "")
            if mesh_path == "": continue
            mesh = trimesh.load_mesh(mesh_path)
            add_name = link_name if name == "" else name + "_" + link_name
            self.add_mesh(utils_3d.transform_points(mesh.vertices, Tw_w2B @ pose), mesh.faces, name=add_name)
    ...
    @property
    def baxter_sk(self):
        if Vis3D._baxter_sk is None:
            from crc.structures.sapien_kin import SAPIENKinematicsModelStandalone
            urdf_path = "./baxter_common/baxter.urdf"
            sk = SAPIENKinematicsModelStandalone(urdf_path)
            Vis3D._baxter_sk = sk
        return Vis3D._baxter_sk
    ...
    @property
    def mobilerobot_sk(self):
        if Vis3D._mobilerobot_sk is None:
            from crc.structures.sapien_kin import SAPIENKinematicsModelStandalone
            urdf_path = "data/mobilerobot/mycobot_pi_v2/mycobot_urdf_stl.urdf"
            sk = SAPIENKinematicsModelStandalone(urdf_path)
            Vis3D._mobilerobot_sk = sk
        return Vis3D._mobilerobot_sk

    def add_mobilerobot(self, qpos, Tw_w2B=None, add_local_coord=False, name=""):
        """
        Parameters
        ----------
        qpos: 6-dim ndarray, radian,
        Tw_w2B: base in world
        add_local_coord
        name
        Returns
        ...
        for link in range(num_links):
            ...
            mesh_path = fixed_link_name_mesh_path_mapping.get(link_name, "")
            if mesh_path == "": continue
            mesh = trimesh.load_mesh(mesh_path)
            add_name = link_name if name == "" else name + "_" + link_name
            self.add_mesh(utils_3d.transform_points(mesh.vertices, Tw_w2B @ pose), mesh.faces, name=add_name)
    ...
    def add_realman(self, qpos=None, Tw_w2B=None, add_local_coord=False, name=""):
        """

        Parameters
        ----------
        qpos: ?-dim ndarray, radian
        Tw_w2B: base in world
        add_local_coord
        name
        ...
        if qpos is None:
            qpos = np.zeros(19)
        qpos = to_array(qpos).tolist() + [0] * (19 - len(qpos))
        sk = self.realman_sk
        links = sk.robot.get_links()
        num_links = len(links)
        local_pts = np.array([[0, 0, 0],
