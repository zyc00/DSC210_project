...
class HandEyeSolverEIH(nn.Module):
    ...
    def forward(self, dps):
        ...
        Tc_c2e, data_num = self.calculate_and_show(Tc_c2ms, dps, ref_Tc_c2e)
        # self.debug(Tb_b2c, ref_Tb_b2c)
        # self.vis_base_origin_in_cam(Te_e2c, dps)
        # self.vis_on_testset(Tb_b2c)
        # print()
        output = {'Tc_c2e': torch.from_numpy(Tc_c2e).cuda().float(),
                  'data_num': torch.tensor(data_num).cuda().float()}
        Tb_b2e0 = dps['link_poses'][0]
        Tb_b2m = Tb_b2e0.cpu().numpy() @ np.linalg.inv(Tc_c2e) @ Tc_c2ms[0]
        output['Tb_b2m'] = torch.from_numpy(Tb_b2m).cuda().float()
        exit(0)
        return output, {}
    ...
    def calculate_marker_pose(self, dps):
        ...
        for i in tqdm.trange(imgs.shape[0]):
            ...
            if self.cfg.chessboard is True:
                criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
                objp = np.zeros((3 * 4, 3), np.float32)
                objp[:, :2] = np.mgrid[0:3, 0:4].T.reshape(-1, 2) / 100.0 * 5
                ret, corners = cv2.findChessboardCornersSB(gray, (3, 4), cv2.CALIB_CB_ACCURACY)
                if ret is False:
                    Tc_c2ms.append(np.zeros((4, 4)))
                ...
                rows, cols = 6, 7
                marker_size, marker_sep = 0.05, 0.02
                objp = []
                for r in range(rows):
                    for c in range(cols):
                        x0 = c * (marker_size + marker_sep)
                        y0 = r * (marker_size + marker_sep)
                        x1 = x0 + marker_size
                        y1 = y0
                        x2 = x0 + marker_size
                        y2 = y0 + marker_size
                ...
                ids = ids[keep]
                corners = np.stack(corners)[keep]
                corners = corners[:, 0, :, :].reshape(-1, 2)
                ret, rvecs, tvecs = cv2.solvePnP(objp[ids].reshape(-1, 3), corners, K, None)
                R, _ = cv2.Rodrigues(rvecs)
                Tc_c2m = utils_3d.Rt_to_pose(R, tvecs[:, 0])

                corners = corners[:, None, :]  # for vis

            plt.figure(dpi=200)
            plt.imshow(img)
            plt.scatter(corners[:, 0, 0], corners[:, 0, 1], s=2, marker='.')
            ...
            vis3d.add_image(np.array(Image.open(img_buf))[:, :, :3], name='aruco')
    ...
    def calculate_and_show(self, Tc_c2ms, dps, ref_Tc_c2e=None):
        ...
        log_file = loguru.logger.add(osp.join(self.total_cfg.sim_hec_eye_in_hand.outdir, "log.txt"), rotation="1 day",
                                     level="DEBUG")
        loguru.logger.debug(f"R_err {R_err:.4f} degree, t_err {t_err:.4f} cm")
        loguru.logger.remove(log_file)

        return Tc_c2e, data_num
    ...
    def __calculate(self, Hee2base, Hmarker2cam):
        ...
        error = np.hstack((res_translation, np.rad2deg(res_rotation)))
        return Hc2g, error
