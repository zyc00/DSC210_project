from crc.utils import opengl_utils
import numpy as np
from crc.utils import transform
...
class ICPRefiner:
    ...
    def refine(self, depth_crop, R_est, t_est, K_test, depth_only=False, no_depth=False, max_mean_dist_factor=2.0):
        vis3d = Vis3D(xyz_pattern=("x", "-y", "-z"), out_folder="dbg", sequence_name="icp_refine", auto_increase=True,
                      enable=True)
        depth = self.renderer.render(self.im_size, 100, 10000, K_test, R_est, t_est)
        synthetic_pts = rgbd_to_point_cloud(K_test, depth)
        vis3d.add_point_cloud(synthetic_pts, name='synthetic_pts')
        centroid_synthetic_pts = np.mean(synthetic_pts, axis=0)
        vis3d.add_point_cloud(centroid_synthetic_pts, name='centroid_synthetic_pts')
        try:
        ...
        vis3d.add_point_cloud(real_depth_pts, name='real_depth_pts')
        if len(real_depth_pts) < len(synthetic_pts) / 20.:
            ...
            T, distances, iterations = icp(synthetic_pts[sub_idcs_syn], real_depth_pts[sub_idcs_real],
                                           tolerance=0.0000005, depth_only=depth_only, no_depth=no_depth)
