...
from crc.structures.safe_dict import SafeDict
...
def slice_outputs(outputs, l):
    if isinstance(outputs, torch.Tensor):
        return outputs[:l]
    elif isinstance(outputs, dict):
        return {k: slice_outputs(v, l) for k, v in outputs.items()}
    else:
        print()
