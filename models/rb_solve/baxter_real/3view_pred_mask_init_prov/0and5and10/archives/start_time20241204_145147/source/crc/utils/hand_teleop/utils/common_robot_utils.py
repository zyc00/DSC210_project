    ...
    if "free" in robot_name:
        for joint in robot.get_active_joints():
            ...
            if "x_joint" in name or "y_joint" in name or "z_joint" in name:
                joint.set_drive_properties(*(1 * root_translation_control_params), mode="force")
            elif "x_rotation_joint" in name or "y_rotation_joint" in name or "z_rotation_joint" in name:
                joint.set_drive_properties(*(1 * root_rotation_control_params), mode="force")
            else:
                joint.set_drive_properties(*(1 * finger_control_params), mode="force")
    elif "xarm" in robot_name:
        arm_joint_names = [f"joint{i}" for i in range(1, 8)]
        for joint in robot.get_active_joints():
            name = joint.get_name()
            if name in arm_joint_names:
                joint.set_drive_properties(*(1 * robot_arm_control_params), mode="force")
            else:
                joint.set_drive_properties(*(1 * finger_control_params), mode="force")
    else:
        raise NotImplementedError
